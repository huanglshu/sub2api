...
func TestFetchOpenAIAccountModelsAPIKeyIncludesExplicitMappings(t *testing.T) {
	gateway := newCodexModelsAPIKeyTestService(&codexModelsHTTPUpstreamStub{do: func(_ *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
		return ordinaryModelsUpstreamResponse(`{"data":[{"id":"upstream-listed"},{"id":"glm-5.3-flash"},{"id":"upstream-custom"}]}`), nil
	}})
	svc := &AccountTestService{openaiGatewayService: gateway}
	...
	require.Equal(t, "custom-alias", models[len(models)-1].ID, "alias keeps the public name")
	require.Equal(t, "custom-alias", models[len(models)-1].DisplayName, "public alias uses itself as the display name")
...
func TestFilterOpenAIAccountTestModelsExpandsWildcardMappings(t *testing.T) {
	gateway := newCodexModelsAPIKeyTestService(&codexModelsHTTPUpstreamStub{do: func(_ *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
		return ordinaryModelsUpstreamResponse(`{"data":[{"id":"glm-5.3"},{"id":"glm-5.3-flash"},{"id":"deepseek-v4-flash"}]}`), nil
	}})
	svc := &AccountTestService{openaiGatewayService: gateway}
	account := newCodexModelsAPIKeyTestAccount("https://models.example/v1")
	account.Credentials["model_mapping"] = map[string]any{"glm-5.3": "glm-5.3", "glm-5.3-flash": "glm-5.3-flash"}
...
func TestFetchOpenAIAccountModelsOAuthLabelsLocalImageModelsLikeUpstream(t *testing.T) {
	...
	account := newCodexModelsTestAccount()
	// 测试选择器遵循 model_mapping 白名单，非空映射会过滤未列出的上游模型；
	// 这里把待断言的 slug 一并列入，否则断言的是被过滤掉的空标签。
	// The picker is scoped to the account mapping, so an unconfigured slug is
	// intentionally absent from the results (see the exclusion tests below).
	account.Credentials["model_mapping"] = map[string]any{
